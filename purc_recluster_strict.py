#!/usr/bin/env python
# This script modified from purc_recluster.py using make_purc_recluster_strict by Joel Nitta, joelnitta@gmail.com
...
def deChimeIt(file, round, abskew=1.9, verbose_level=0):
	...
	usearch_cline = "%s -uchime_denovo %s -abskew 1.1 -minh 0.2 -xn 3 -dn 0.5 -nonchimeras %s -uchimeout %s -threads %s" % (Usearch, file, outFile, outFile_uchime, num_threads)
	process = subprocess.Popen(usearch_cline, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)	
	(out, err) = process.communicate() #the stdout and stderr
	savestdout = sys.stdout 
